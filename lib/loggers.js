...
//
const loggers = {
  error: logger('solarwinds-apm:error'),
  warn: logger('solarwinds-apm:warn'),
  debug: logger('solarwinds-apm:debug'),
  span: logger('solarwinds-apm:span'),
  info: logger('solarwinds-apm:info'),
  patching: logger('solarwinds-apm:patching'), // show errors patching
  probes: logger('solarwinds-apm:probes'), // show probe information
  bind: logger('solarwinds-apm:bind') // show binding errors
}

initial && initial.split(',').forEach(level => { loggers[level].enabled = true })
...
loggers.addGroup = function (def) {
  ...
  const g = {}
  const groupPrefix = 'solarwinds-apm:' + def.groupName + ':'

  def.subNames.forEach(function (name) {
    g[name] = logger(groupPrefix + name)
  })
